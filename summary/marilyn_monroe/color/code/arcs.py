...
def initialization(population_number: int):
    return [Image.new('RGB', img_size, (255, 255, 255)) for i in range(population_number)]
...
def main():
    ...
    while generation < NUMBER_OF_GENERATIONS:
        ...
        if (generation % 500) == 0:
            ...
            out_file = open("../results/marilyn_monroe_" + str(generation) + ".jpg", 'wb')
            best_solution.save(out_file, "JPEG")
            out_file.flush()
            out_file.close()

            for i in range(POPULATION_NUMBER):
                np.save("../populations/population_marilyn_monroe_" + str(i), np.asarray(population[i]))
...
img = Image.open("../../../../images/marilyn_monroe.jpg")
img_size = img.size
