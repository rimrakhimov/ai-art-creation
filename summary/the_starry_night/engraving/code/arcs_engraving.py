...
def mutation(population):
    ...
    colors= np.random.random_integers(0, 255, mutation_number)
...
def main():
    ...
    while generation < NUMBER_OF_GENERATIONS:
        ...
        if (generation % 500) == 0:
            ...
            out_file = open("../results/the_starry_night_" + str(generation) + ".jpg", 'wb')
            best_solution.save(out_file, "JPEG")
            out_file.flush()
            out_file.close()

            for i in range(POPULATION_NUMBER):
                np.save("../populations/population_the_starry_night_" + str(i), np.asarray(population[i]))
...
img = Image.open("../../../../images/the_starry_night.jpg")
img_size = img.size
