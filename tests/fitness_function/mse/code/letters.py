...
def mutation(population):
    ...
    locations = np.random.randint(-10, max(img_size), (mutation_number, 2))
    indexes = np.random.randint(0, len(LETTERS), mutation_number)
...
def main():
    ...
    for i in range(POPULATION_NUMBER):
        fit_values[i] = fitness_function_mse(population[i])
    ...
    while generation < NUMBER_OF_GENERATIONS:
        ...
        if (generation % 500) == 0:
            ...
            out_file = open("../results/letters/mona_lisa_" + str(generation) + ".jpg", 'wb')
            best_solution.save(out_file, "JPEG")
            out_file.flush()
            out_file.close()

            for i in range(POPULATION_NUMBER):
                np.save("../populations/letters/population_mona_lisa_" + str(i), np.asarray(population[i]))
        ...
        for i in range(POPULATION_NUMBER):
            new_fit_values[i] = fitness_function_mse(new_population[i])
