...
def mutation(population):
    ...
    colors = np.random.randint(0, 256, (mutation_number, 3), dtype=np.uint8)                # the color of triangle
    initial_points = np.random.random_integers(-5, max(img_size), (mutation_number, 2))     # position of initial point of the triangle
    triangle_sizes = np.random.random_integers(20, 70, (mutation_number, 2, 2))             # 2 variables which will be added to the initial point in order to get 2 other points of the triangle
...
def main():
    ...
    while generation < NUMBER_OF_GENERATIONS:
        ...
        if (generation % 500) == 0:
            ...
            out_file = open("../results/output_triangles.jpg", 'wb')
            ...
            for i in range(POPULATION_NUMBER):
                np.save("../populations/population_output_triangles_" + str(i), np.asarray(population[i]))
