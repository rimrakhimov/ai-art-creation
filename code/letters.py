...
def mutation(population):
    ...
    colors = np.random.randint(0, 256, (mutation_number, 3), dtype=np.uint8)    # the color of the letter
    locations = np.random.randint(-10, max(img_size), (mutation_number, 2))     # the coordinate where letter will be added
    indexes = np.random.randint(0, len(LETTERS), mutation_number)               # the letter to add
    ...
    for i in range(POPULATION_NUMBER):
        if indicators[i]:   # if chromosome should mutate
            ...
            # draws color letter
            draw.text(locations[count], LETTERS[indexes[count]], fill=tuple(colors[count]), font=font)
            del draw
            count += 1
...
def main():
    ...
    while generation < NUMBER_OF_GENERATIONS:
        ...
        if (generation % 500) == 0:
            ...
            out_file = open("../results/output_letters.jpg", 'wb')
            ...
            for i in range(POPULATION_NUMBER):
                np.save("../populations/population_output_letters_" + str(i), np.asarray(population[i]))
...
# upload fonts used for letters into the memory
