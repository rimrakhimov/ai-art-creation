...
def mutation(population):
    ...
    colors = np.random.randint(0, 256, (mutation_number, 3), dtype=np.uint8)                # color of the arc
...
def main():
    ...
    while generation < NUMBER_OF_GENERATIONS:
        ...
        if (generation % 500) == 0:
            ...
            out_file = open("../results/output_arcs.jpg", 'wb')
            ...
            for i in range(POPULATION_NUMBER):
                np.save("../populations/population_output_arcs_" + str(i), np.asarray(population[i]))
